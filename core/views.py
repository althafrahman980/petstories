...
def index(request):
    ...
    user_following_list = FollowerCount.objects.filter(follower= request.user.username)
    
    for x in user_following_list:
        feed_list_usernames.append(x.user)
    
    for usernames in feed_list_usernames:
        feed_post = Post.objects.filter(user= usernames)
        feed_list.append(feed_post)

    feed = list(chain(*feed_list))

    #Userprofiles suggestions
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                messages.info(request , 'Ooops! Email already in use!')
                return redirect('signup')
            elif User.objects.filter(username = username).exists():
                messages.info(request , 'Oops! Username already in use!')
                return redirect('signup')
            else:
            ...
            messages.info(request, 'Ooops! Password not matching, please try again!')
            return redirect('signup')
...
@login_required(login_url='follow')
def follow(request):
    if request.method == 'POST':
